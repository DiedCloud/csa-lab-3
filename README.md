...
* Обработка данных осуществляется в стеке. Данные попадают в стек из памяти (устройств ввода/вывода привязаны к ячейкам памяти), либо с помощью инструкции LIT.
* Поток управления:
...
* `JUMP { address }` – безусловный переход по указанному адресу.
